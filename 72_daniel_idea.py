...
def ind_def_evaluator_2(defender, receiver, ball):
    ...
    return np.sum(np.sum(def_control, axis = 1), axis = 0) / (np.sum(np.sum(off_control, axis = 1), axis = 0) + np.sum(np.sum(def_control, axis = 1), axis = 0))
...
# final_throw_df_2.write_parquet("sharing/final_throw_df_3.parquet")
...
def build_arrival_final_3():
    ...
    for index, row, in game_play_defender_trios_3.to_pandas().iterrows():
        GS = row["game_id"]
        PS = row["play_id"]
        Def_Id = row["nfl_id"]
        defender_control[index] = ind_def_evaluator_2(
